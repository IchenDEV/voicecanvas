...
| 能力 | 状态 | 说明 |
| --- | --- | --- |
| 语音优先指令链路 | 原型可用 | 支持文本片段，也可以选择 OpenAI Realtime 或 Gemini Live 语音输入。 |
...
| 实时语音服务 | 可选 | OpenAI Realtime 走 API 代理，Gemini Live 走短期浏览器 token。 |
...
| `pnpm check:alpha:realtime` | 运行实时语音服务 API 测试。 |
...
| `POST` | `/api/realtime/gemini/token` | 签发短期 Gemini Live 浏览器 token。 |
