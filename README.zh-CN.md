...
| 能力 | 状态 | 说明 |
| --- | --- | --- |
| 语音优先指令链路 | 原型可用 | 支持文本片段，也支持可选实时 ASR 输入。 |
...
| 豆包实时 ASR 代理 | 可选 | 配置 key 后由 API 服务代理麦克风音频。 |
...
模型在这里负责提出 Patch 草稿。画布只会在草稿通过校验、Patch 引擎执行后变化。这个拆分让图状态更容易检查，撤回更可靠，也方便后续替换 ASR 或模型服务。
...
Vite 开发服务会把 `/api` 和实时 WebSocket 请求代理到 API 服务。外部模型配置为空时，项目会使用内置 Mock Patch 编译器。
...
| `GET` | `/api/realtime/provider` | 查看实时 ASR 服务配置。 |
